...
module SidekiqExtensions
	...
	def self.prioritize_queue(queue_name, position = {})
		validate_parameters(queue_name, position)
		queue_name = queue_name.to_s
		insert_at = insertion_index(queue_name, position.keys[0], position.values[0].to_s)
		remove_queue(queue_name, :from => [:priority_queues, :queues])
		Sidekiq.options[:priority_queues].insert(insert_at, queue_name)
	end
	...
	def self.validate_parameters(queue_name, position)
		...
		raise ArgumentError, 'Queue cannot be positioned relative to itself' if position.values.first == queue_name.to_s
	end
